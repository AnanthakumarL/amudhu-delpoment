...
class AccountService:
    ...
    def list(self, page: int = 1, page_size: int = 20, is_active: bool | None = None) -> tuple:
        try:
            q = self.db.query(AccountORM)
            if is_active is not None:
                q = q.filter(AccountORM.is_active == is_active)
            q = q.order_by(AccountORM.created_at.desc())
            total = q.count()
