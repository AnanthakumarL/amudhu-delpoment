...
class OrderService:
    ...
    def get_statistics(self) -> OrderStatistics:
        try:
            stats = {"total_orders": 0, "pending_orders": 0, "processing_orders": 0,
                     "shipped_orders": 0, "delivered_orders": 0, "cancelled_orders": 0, "total_revenue": 0.0}
            for row in self.db.query(OrderORM).all():
                stats["total_orders"] += 1
                s = row.status
                if s == "pending": stats["pending_orders"] += 1
                elif s == "processing": stats["processing_orders"] += 1
                elif s == "shipped": stats["shipped_orders"] += 1
                elif s == "delivered":
                    stats["delivered_orders"] += 1
                    stats["total_revenue"] += float(row.total or 0)
                elif s == "cancelled": stats["cancelled_orders"] += 1
